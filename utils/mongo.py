...
class MongoStorage(BaseStorage):
    ...
    async def set_state(self, bot: Bot, key: StorageKey, state: StateType = None) -> None:
        ...
        self.collection.update_one({'key': key.__dict__}, {'$set': {"state":final_state}}, upsert=True)


    async def get_state(self, bot: Bot, key: StorageKey) -> Optional[str]:
        data = self.collection.find_one({'key': key.__dict__})
        if data:
            return data.get('state')
        return None

    async def set_data(self, bot: Bot, key: StorageKey, data: Dict[str, Any]) -> None:
        self.collection.update_one({'key': key.__dict__}, {'$set': {"data":data}}, upsert=True)
    ...
    async def update_data(
        ...
        if data:
            kwargs.update(data)
        self.collection.update_one({'key': key.__dict__}, {'$set': {"data":kwargs}}, upsert=True)
        return kwargs
